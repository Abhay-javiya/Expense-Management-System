...
import { Plus, Search, Filter, Edit2, Trash2, User, Mail, Phone, Info, Layout } from "lucide-react";
...
export default function PeoplePage() {
  ...
  const fetchPeoples = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || "Failed to load people data");
    } finally {
      setLoading(false);
    }
  };
  ...
  const deletePeople = async (id: number) => {
    if (!confirm("Remove this person from your contacts?")) return;
  ...
  return (
    <div className="p-4 sm:p-8 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">People</h1>
          <p className="text-sm text-slate-500 font-medium">Manage your network of contacts and beneficiaries</p>
        </div>
        <button
      ...
      <div className="rounded-[32px] bg-white shadow-sm border border-slate-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse min-w-[700px] sm:min-w-0">
            ...
            <tbody className="divide-y divide-slate-50">
              {peoples.length === 0 ? (
                ...
                peoples.map((p) => (
                  <tr key={p.PeopleID} className="group hover:bg-indigo-50/20 transition-colors cursor-pointer">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-slate-800 truncate">{p.PeopleName}</p>
                    ...
                    {/* Column 2: Contact Info (Hidden on Small) */}
                    ...
                    <td className="px-4 py-4">
                      <span className="inline-flex px-2.5 py-1 rounded-lg bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-wider">
                        Active
                      </span>
                    </td>
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          onClick={() => router.push(`/user/peoples/edit/${p.PeopleID}`)}
        ...
        <div className="p-4 sm:p-6 bg-slate-50/30 border-t border-slate-50 flex justify-center">
          <p className="text-xs font-bold text-slate-400">Total {peoples.length} members</p>
        </div>
      </div>
    </div>
  );
